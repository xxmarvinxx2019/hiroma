...
function ResetPasswordForm() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!token)
      return setError(
        "This reset link is invalid. Request a new link from your administrator.",
      );
  ...
  const PasswordInput = ({ confirm = false }: { confirm?: boolean }) => {
    ...
    const toggle = confirm ? setShowConfirmPassword : setShowPassword;
    return (
      <label className="block text-sm font-medium text-[#0D1B3E]">
        ...
        <span className="relative mt-1.5 block">
          <input
            ...
            minLength={8}
            required
            className="w-full rounded-lg border border-[#0D1B3E]/15 bg-[#F0F2F8] px-3 py-2.5 pr-12 outline-none focus:border-[#C9A84C]"
          />
          <button
            type="button"
            onClick={() => toggle((current) => !current)}
            className="absolute inset-y-0 right-0 flex w-11 items-center justify-center text-gray-400 hover:text-[#C9A84C]"
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#F0F2F8] p-4">
      <section className="w-full max-w-md rounded-2xl border border-[#0D1B3E]/10 bg-white p-6 shadow-xl">
        ...
        <h1 className="mt-2 text-2xl font-semibold text-[#0D1B3E]">
          Set a new password
        </h1>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          {error && (
            <p className="rounded-lg bg-[#fdecea] px-3 py-2 text-sm text-[#a03030]">
              {error}
            </p>
          )}
          {success && (
            <p className="rounded-lg bg-[#e8f7ef] px-3 py-2 text-sm text-[#1a7a4a]">
              {success}
            </p>
          )}
          <button
            disabled={loading || Boolean(success)}
            className="w-full rounded-lg bg-[#C9A84C] py-2.5 font-medium text-white hover:bg-[#b8963e] disabled:opacity-50"
          >
        ...
        <Link
          href="/login"
          className="mt-5 block text-center text-sm text-[#9a6f1e] hover:underline"
        >
          Back to sign in
        </Link>
      </section>
