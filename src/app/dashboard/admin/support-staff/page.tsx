    ...
    <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      <form onSubmit={submit} className="space-y-4 rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {(['full_name','username','mobile','email','password'] as const).map((key) => <PasswordInput key={key} required={key !== 'email'} type={key === 'password' ? 'password' : key === 'email' ? 'email' : 'text'} minLength={key === 'password' ? 8 : undefined} value={form[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} placeholder={key.replace('_', ' ')} className={`${key === 'password' ? 'sm:col-span-2 ' : ''}rounded-xl border px-3 py-2.5 text-sm`} />)}
        </div>
