...
export default function AdminRegisterResellerPage() {
  ...
  const fetchPins = useCallback(
    (id?: string) => {
      ...
      setPinsLoading(true)
      fetch(
        `/api/admin/pins?status=unused&all_dates=true&pageSize=200&city_dist_id=${encodeURIComponent(distId)}`,
      )
        .then((r) => r.json())
  ...
  const handleRegister = async () => {
    if (
      ...
      setFormError('Please fill in all required fields including email.')
      return
    }
    ...
    const res = await fetch('/api/admin/resellers/register', {
      ...
      body: JSON.stringify({
        ...
        street_address: location.street.trim(),
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <button
          onClick={() => router.push('/dashboard/admin/resellers')}
        ...
        <p className="text-sm text-gray-400 mt-0.5">
          Register a new reseller directly as admin
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-8">
        {['PIN', 'Location', 'Referral & Slot', 'Details'].map((label, i) => (
          <div key={label} className="flex items-center gap-2 flex-1">
            ...
            <span
              className={`text-xs font-medium ${step === i + 1 ? 'text-[#0D1B3E]' : 'text-gray-400'}`}
            >
              {label}
            </span>
      ...
      <div className="bg-white rounded-2xl border border-[#0D1B3E]/8 p-6">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-4">
            <div>
              <h3 className="text-sm font-semibold text-[#0D1B3E] mb-1">
                Select PIN
              </h3>
              <p className="text-xs text-gray-400">
                Select an unused PIN from your generated PINs.
              </p>
            </div>
                ...
                {filteredPins.map((pin) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-mono font-semibold text-[#0D1B3E]">
                          {pin.pin_code}
                        </p>
                        <p className="text-[10px] text-gray-400">
                          {pin.package?.name} · ₱
                          {Number(pin.package?.price || 0).toLocaleString()}
                        </p>
                      </div>
                      {pinData?.id === pin.id && (
            ...
            {pinData && (
              <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-lg px-4 py-3">
                <p className="text-xs text-[#1a7a4a] font-semibold">
                  ✓ PIN selected: {pinData.pin_code}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  Package: {pinData.package?.name} · ₱
                  {Number(pinData.package?.price || 0).toLocaleString()}
                </p>
              </div>
            )}
            ...
            <button
              onClick={() => {
                if (!pinData) {
                  setPinError('Please select a PIN.')
                  return
                }
                setStep(2)
              }}
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-lg px-4 py-3">
              <p className="text-xs text-[#1a7a4a] font-semibold">
                ✓ PIN: {pinData?.pin_code} · {pinData?.package?.name}
              </p>
            </div>
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">
                City / Municipality <span className="text-[#C9A84C]">*</span>
              </label>
              <select
            ...
            {/* Official barangay — controlled by selected city/municipality */}
            <div>
              <label className="block text-xs text-gray-400 mb-1">
                Barangay <span className="text-[#C9A84C]">*</span>
              </label>
              <select
                value={location.barangay_code}
                disabled={!location.city_muni_code || loadingBarangays}
                onChange={(e) => {
            ...
            {/* Detailed address and official ZIP */}
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_140px] gap-3">
              <div>
                ...
                <input
                  ...
                  required
                  onChange={(e) =>
                    setLocation((l) => ({ ...l, street: e.target.value }))
                  }
                ...
                <div className="bg-[#F0F2F8] rounded-lg px-4 py-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {referralData.daily_cap_reached && (
                      <p className="text-xs text-[#9a6f1e] mt-0.5">
                        ⚠️ Daily cap reached — overflow to Hiroma
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                          ...
                          {availableSlots
                            .filter(
                              (s) =>
                                ...
                                s.full_name
                                  .toLowerCase()
                                  .includes(slotSearch.toLowerCase()) ||
                                s.username
                                  .toLowerCase()
                                  .includes(slotSearch.toLowerCase()),
                            )
                            .map((slot) => (
                              <div
                                key={slot.node_id}
                                className="px-3 py-2 border-b border-[#0D1B3E]/5 last:border-0 hover:bg-[#F0F2F8]"
                              >
                                <p className="text-xs font-medium text-[#0D1B3E]">
                                  {slot.full_name}
                                </p>
                                <p className="text-[10px] text-gray-400 mb-1">
                                  @{slot.username} · {slot.package}
                                </p>
                  ...
                  {selectedSlot && (
                    <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-lg px-3 py-2 mt-2">
                      <p className="text-xs text-[#1a7a4a] font-semibold">
                        ✓ Placement: {selectedSlot.position} leg under @
                        {selectedSlot.parent_username}
                      </p>
                    </div>
                  )}
                </div>
        ...
        {step === 4 && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-lg px-4 py-3">
              <p className="text-xs text-[#1a7a4a] font-semibold mb-1">
                ✓ All confirmed
              </p>
            ...
            <div>
              ...
              {nameCapInfo?.error && (
                <p className="text-xs text-red-500 mt-1">{nameCapInfo.error}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Date of Birth <span className="text-[#C9A84C]">*</span>
                </label>
                <input
                  type="date"
                  value={form.birthday}
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Place of Birth <span className="text-[#C9A84C]">*</span>
                </label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Valid ID <span className="text-[#C9A84C]">*</span>
                </label>
                <select
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  ID number <span className="text-[#C9A84C]">*</span>
                </label>
                <input
                ...
                />
                <p className="text-[11px] text-gray-400 mt-1">
                  Used only to match the member securely; the number is not
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Temporary password <span className="text-[#C9A84C]">*</span>
                </label>
                <input
                  type="text"
                  value={form.password}
                  readOnly
                  placeholder="Generated after identity details"
                  className="w-full bg-gray-100 border border-[#0D1B3E]/15 rounded-lg px-3 py-2 text-sm font-medium text-[#0D1B3E] outline-none cursor-not-allowed"
                />
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-3 text-xs text-gray-400">
              Verify the presented valid ID before confirming. No password, PIN,
              or ID number is shown here.
            </p>
      ...
      {successData && smsPromptOpen && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            <div className="w-12 h-12 rounded-full bg-[#fef9ee] flex items-center justify-center mx-auto text-2xl">
              📱
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center px-4 overflow-y-auto py-6">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden my-auto">
            ...
            <div className="bg-[#010521] px-6 py-6 text-center relative">
              ...
              <p className="text-[#C9A84C] text-[10px] tracking-widest uppercase mb-1 relative">
                Welcome to Hiroma
              </p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-xl px-4 py-3">
                <p className="text-xs font-semibold text-[#1a7a4a] mb-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium text-[#0D1B3E] font-mono">
                      @{successData.username}
                    </span>
                  </div>
                  <div className="flex justify-between gap-4 text-xs">
                    <span className="text-gray-500 shrink-0">Address</span>
                    <span className="font-medium text-[#0D1B3E] text-right">
                      {successData.address}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                ...
                <>
                  ...
                  <div className="flex items-center justify-between bg-[#fef9ee] border border-[#C9A84C]/20 rounded-xl px-4 py-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-bold text-[#C9A84C]">
                        ₱
                        {Number(
                          successData.package.total_price,
                        ).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#F0F2F8] rounded-xl px-4 py-3 space-y-2">
                    ...
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium text-[#0D1B3E]">
                        ₱
                        {Number(successData.package.pin_price).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium text-[#0D1B3E]">
                        ₱
                        {Number(
                          successData.package.products_total,
                        ).toLocaleString()}
                      </span>
                    </div>
                  ...
                  {successData.package.products?.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {successData.package.products.map(
                          (p: any, i: number) => (
                            <div
                              key={i}
                              className="flex items-center justify-between bg-[#F0F2F8] rounded-lg px-3 py-2"
                            >
                              ...
                              <span className="text-xs font-semibold text-[#0D1B3E] ml-2">
