...
export default function OrderDetailPage() {
  ...
  if (error || !order) return (
    <div className="max-w-5xl mx-auto py-12 text-center">
      ...
      <button onClick={() => router.push('/dashboard/admin/orders')} className="text-xs text-[#C9A84C] hover:underline">← Go back</button>
    </div>
  )
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .order-no .label { font-size: 12px; color: #888; }
    .order-no .value { font-weight: bold; color: #0D1B3E; }
    .badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 11px; font-weight: bold;
      background: ${order.status === 'delivered' ? '#e8f7ef' : '#fef9ee'}; 
      color: ${order.status === 'delivered' ? '#1a7a4a' : '#9a6f1e'}; }
    ...
    .info-box .title { font-size: 10px; color: #888; text-transform: uppercase; margin-bottom: 6px; letter-spacing: 0.05em; }
...
<body>
  ...
  <div class="order-no">
    <div>
      ...
      <div class="value">${order.order_number || `#${order.id.slice(0,8).toUpperCase()}`}</div>
    </div>
    <div style="text-align:right;">
      <div class="label">Date</div>
      <div class="value">${new Date(order.created_at).toLocaleDateString('en-PH', { month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
    </div>
    <div style="text-align:right;">
      <span class="badge">${order.status.charAt(0).toUpperCase() + order.status.slice(1)}</span>
    </div>
  </div>
  ...
  <div class="info-grid">
    <div class="info-box">
      ...
      <div class="sub">${order.buyer?.address || ''}</div>
    </div>
    <div class="info-box">
  ...
  <div class="footer">
    ...
    <p style="margin-top:4px;">This is an official receipt from Hiroma.</p>
  </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => router.push('/dashboard/admin/orders')}
            className="text-xs text-gray-400 hover:text-[#0D1B3E] transition-colors flex items-center gap-1">
            ← Back to Orders
          </button>
