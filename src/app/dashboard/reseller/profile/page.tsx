...
export default function ResellerProfilePage() {
  ...
  const saveProfile = async (securityPin = ''): Promise<string | null> => {
    ...
    const res = await fetch('/api/reseller/profile', {
      ...
      body: JSON.stringify({ ...profileForm, profile_photo: profilePhoto, security_pin: securityPin }),
    })
    ...
    if (res.ok) {
      ...
      window.dispatchEvent(new CustomEvent('hiroma-reseller-profile-photo-change', { detail: data.user?.profile_photo ?? profilePhoto }))
      setPinAction(null)
      return null
    }
  ...
  const handlePhotoChange = (file: File | undefined) => {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 1_000_000) {
      setPhotoError('Choose a JPEG, PNG, or WebP image up to 1 MB.')
      return
    }
    const reader = new FileReader()
    reader.onload = () => setProfilePhoto(typeof reader.result === 'string' ? reader.result : null)
    reader.onerror = () => setPhotoError('Unable to read that image. Please try another photo.')
      ...
      <Section title="Personal Information" desc="Update your name, contact details, and address">
        <div className="mb-5 flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-gray-400">JPEG, PNG, or WebP. Maximum 1 MB.</p>
            <div className="mt-2 flex items-center gap-3">
              <label className="cursor-pointer text-xs font-medium text-[#C9A84C] hover:underline">
                {profilePhoto ? 'Change photo' : 'Add photo'}
                <input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => handlePhotoChange(event.target.files?.[0])} />
              </label>
              {profilePhoto && <button type="button" onClick={() => setProfilePhoto(null)} className="text-xs font-medium text-gray-500 hover:text-[#a03030]">Remove</button>}
            </div>
