...
export default function ProvincialInventoryPage() {
  ...
  const saveThreshold = async (id: string) => {
    setSaving(true)
    await fetch('/api/provincial/inventory', {
      method:  'PATCH',
    ...
    <div className="w-full space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Manage your provincial stock levels and product movement</p>
        </div>
            ...
            return (
              <div key={item.id} className={`grid grid-cols-6 px-4 py-3.5 border-b border-[#0D1B3E]/5 hover:bg-[#f8f9fc] transition-colors items-center ${!item.product.is_active ? 'opacity-50' : ''}`}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400">{fmt(item.product.provincial_price)} / unit</p>
                </div>
                {/* Type */}
