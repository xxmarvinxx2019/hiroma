...
function CityRegisterResellerPageInner() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-6">
        <button onClick={() => router.push('/dashboard/city/resellers')}
        ...
        <p className="text-sm text-gray-400 mt-0.5">Register a new reseller under your network</p>
      </div>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-5">
            <div>
              ...
              <p className="text-xs text-gray-400">Enter the unused PIN code for the new reseller's package.</p>
            </div>
            <div>
            ...
            <button onClick={verifyPin} disabled={pinLoading || !pinInput.trim()}
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#e8f7ef] border border-[#1a7a4a]/30 rounded-lg px-4 py-3">
              ...
              <p className="text-xs text-gray-400 mt-0.5">₱{Number(pinData?.package?.price || 0).toLocaleString()}</p>
            </div>
                  ...
                  <div className="relative slot-dropdown-container">
                    ...
                    {slotDropdownOpen && !slotsLoading && availableSlots.length > 0 && (
                      <div className="absolute top-full left-0 right-0 z-50 bg-white border border-[#0D1B3E]/15 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                        {availableSlots
                          .filter((s) => !slotSearch.trim() || s.full_name.toLowerCase().includes(slotSearch.toLowerCase()) || s.username.toLowerCase().includes(slotSearch.toLowerCase()))
                          .map((slot) => (
                            <div key={slot.node_id} className="px-3 py-2 border-b border-[#0D1B3E]/5 last:border-0 hover:bg-[#F0F2F8]">
                              ...
                              <p className="text-[10px] text-gray-400 mb-1.5">@{slot.username} · {slot.package}</p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="bg-[#F0F2F8] rounded-xl px-4 py-3">
                ...
                <div className="space-y-1.5">
                  ...
                  ].map((s, i) => <p key={i} className="text-xs text-gray-500">{s}</p>)}
                </div>
              </div>

              <div className="flex gap-2">
                <button onClick={() => { setSuccessData(null); resetForm() }}
