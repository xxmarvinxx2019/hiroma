...
// ============================================================
// CREATE ORDER FOR RESELLER MODAL
// ============================================================

function CreateResellerOrderModal({
  onClose,
  onSuccess,
}: {
  onClose:   () => void
  ...
  useEffect(() => {
    fetch('/api/city/orders/reseller-orders')
      .then((r) => r.json())
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col border-r border-[#0D1B3E]/8 min-w-0">
            ...
            <div className="flex-1 overflow-y-auto">
              {!selectedResellerId ? (
                <p className="text-center text-xs text-gray-400 py-8">Select a reseller first</p>
              ) : loadingProducts ? (
                <div className="flex justify-center py-8">
                  <div className="w-5 h-5 border-2 border-[#C9A84C] border-t-transparent rounded-full animate-spin" />
                </div>
              ) : filtered.length === 0 ? (
                <p className="text-center text-xs text-gray-400 py-8">No products in stock</p>
              ) : (
                filtered.map((product) => (
