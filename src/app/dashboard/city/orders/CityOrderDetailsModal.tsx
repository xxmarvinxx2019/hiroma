  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/55 p-4" role="dialog" aria-modal="true" aria-labelledby="city-order-modal-title" onMouseDown={onClose}>
    <div className="max-h-[92vh] w-full max-w-2xl overflow-hidden rounded-[24px] border border-white/20 bg-[#F7F8FC] shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      <header className="relative flex items-start justify-between overflow-hidden bg-[#010521] px-6 py-5">
        ...
        <button type="button" aria-label="Close order details" onClick={onClose} className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-xl text-white/60 hover:bg-white/20 hover:text-white">×</button>
      </header>
      <div className="max-h-[calc(92vh-88px)] space-y-4 overflow-y-auto p-5">
        ...
        {order.pickup_scheduled_at && <section className="rounded-2xl border border-[#C9A84C]/30 bg-[#fef9ee] p-4 shadow-sm"><p className="text-[10px] uppercase tracking-wide text-[#9a6f1e]">Scheduled pickup</p><p className="mt-1 text-sm font-bold text-[#0D1B3E]">{new Date(order.pickup_scheduled_at).toLocaleString('en-PH', { timeZone: 'Asia/Manila', dateStyle: 'full', timeStyle: 'short' })}</p><p className="mt-1 text-xs text-gray-600">Prepare these reserved items before the reseller arrives. They are excluded from walk-in available stock.</p></section>}
        ...
        {!order.is_non_member_sale && <OrderPaymentPanel orderId={order.id} />}
        {!order.is_non_member_sale && <OrderConversation orderId={order.id} />}
