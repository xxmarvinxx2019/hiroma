...
type LedgerRow = {
  ...
  product_name: string; quantity: number; unit_cost: number | null; total_value: number | null; reference: string
...
type AuditSummary = {
  ...
  totals: { products: number; counted: number; variance_units: number; variance_value: number | null }
}
type AuditItem = {
  id: string; product_name_snapshot: string; expected_quantity: number; counted_quantity: number | null
  variance_quantity: number | null; unit_cost_snapshot: number | null; variance_value: number | null
...
export function InventoryMovementLedger() {
  ...
  const [summary, setSummary] = useState({ stock_in_units: 0, stock_out_units: 0, adjustment_units: 0, net_movement_units: 0, total_movement_value: null as number | null, movement_count: 0, pending_approval: 0, latest_approved_audit: null as null | { reference_number: string; approved_at: string } })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(data.rows || []); setSummary(data.summary || {}); setCanViewFinancials(Boolean(data.access?.can_view_financials))
    ...
    <div className="overflow-hidden rounded-2xl border border-[#0D1B3E]/10 bg-white">
      ...
      {error ? <div className="m-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : loading ? <div className="py-16 text-center text-sm text-gray-400">Loading movement ledger…</div> : rows.length === 0 ? <div className="py-16 text-center text-sm text-gray-400">No movements match these filters.</div> : <>
        ...
        <div className="divide-y divide-[#0D1B3E]/6">{rows.map((row) => <div key={row.id} className="grid gap-3 px-4 py-4 lg:grid-cols-[1.05fr_1.15fr_.55fr_.55fr_.55fr_.75fr_1fr_1.1fr] lg:items-center lg:gap-2">
          ...
          <div><p className="text-[9px] font-bold uppercase tracking-wide text-gray-400 lg:hidden">Movement value</p>{canViewFinancials && row.total_value !== null && row.unit_cost !== null ? <><p className="text-xs font-bold text-[#0D1B3E]">{money(row.total_value)}</p><p className="text-[9px] text-gray-400">{money(row.unit_cost)} per unit</p></> : <p className="text-[10px] font-semibold text-gray-400">Reports access required</p>}</div>
        ...
        <div className="border-t-2 border-[#0D1B3E]/10 bg-[#f8f9fc] px-4 py-4">
          <div className="grid gap-3 lg:grid-cols-[1.05fr_1.15fr_.55fr_.55fr_.55fr_.75fr_1fr_1.1fr] lg:items-center lg:gap-2">
            ...
            <div className="hidden lg:block" aria-hidden="true">—</div>
            <div><p className="text-[10px] font-bold uppercase tracking-wide text-gray-500 lg:hidden">Total movement value</p>{canViewFinancials && summary.total_movement_value !== null ? <p className="text-sm font-extrabold text-[#0D1B3E]">{money(summary.total_movement_value)}</p> : <p className="text-[10px] font-semibold text-gray-400">Reports access required</p>}</div>
...
export function InventoryPhysicalAudits() {
  ...
  const loadSessions = useCallback(async () => {
    setLoading(true); setError('')
    try { const response = await fetch('/api/city/inventory/audits', { cache: 'no-store' }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setSessions(data.sessions || []); setCanApprove(Boolean(data.can_approve)); setCanViewFinancials(Boolean(data.access?.can_view_financials)) }
  ...
  const openSession = async (id: string) => {
    ...
    setSelected(data.session); setCanApprove(Boolean(data.can_approve)); setCanCount(Boolean(data.can_count)); setCanViewFinancials(Boolean(data.access?.can_view_financials)); setAreaManagerAudit(Boolean(data.is_area_manager_audit))
  ...
  const draftTotals = useMemo(() => selected?.items.reduce((totals, item) => {
    ...
    totals.units += Math.abs(variance)
    if (canViewFinancials && item.unit_cost_snapshot !== null) totals.value += Math.abs(variance * item.unit_cost_snapshot)
    return totals
  }, { units: 0, value: 0 }) || { units: 0, value: 0 }, [canViewFinancials, countDraft, selected])
  ...
  const dialogCopy = dialog ? {
    ...
    submit: { title: 'Submit for Approval?', description: 'The completed counts will be locked and sent to the owner, branch manager, or assigned Operations Approver for independent review.', confirm: 'Submit Count' },
    ...
    approve: { title: 'Approve & Accept Accountability?', description: 'Confirm that you independently verified the physical count. Approval records your identity, reconciles official inventory, and makes you accountable for this decision. Shortages or overages require verification notes.', confirm: 'Approve & Reconcile' },
  ...
  return <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-[#d7ab42]/40 bg-[#fffaf0] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-bold text-[#0D1B3E]">Physical Count & Surprise Audit</h2><p className="mt-1 max-w-3xl text-xs leading-relaxed text-gray-600">Inventory staff count and submit. A different owner, branch manager, or Operations Approver must verify discrepancies before official inventory changes.</p></div><button onClick={() => openDialog('start')} disabled={busy} className="rounded-xl bg-[#0D1B3E] px-5 py-3 text-xs font-bold text-white disabled:opacity-50">+ Start Full Count</button></div>
