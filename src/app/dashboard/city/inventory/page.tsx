...
export default function CityInventoryPage() {
  ...
  const getPrice = (item: InventoryItem) => {
    // Prisma Decimal can come back as string — use Number() and fallback chain
    const p = Number(item.product.reseller_price || item.product.city_price || 0)
    return isNaN(p) ? 0 : p
  }
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Products', value: summary.total_products, accent: '#0D1B3E' },
          { label: 'Total Units',    value: summary.total_units,    accent: '#0D1B3E' },
          { label: 'Low Stock',      value: summary.low_stock,      accent: '#C9A84C' },
          { label: 'Out of Stock',   value: summary.out_of_stock,   accent: '#e05252'  },
        ].map((s) => (
          ...
          items.map((item) => {
            ...
            return (
              <div key={item.id}
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full w-fit ${item.product.type === 'physical' ? 'bg-[#eef0f8] text-[#0D1B3E]' : 'bg-[#f0f7ff] text-[#2563eb]'}`}>
                  {item.product.type}
                </span>
                  ...
                  {isEditing ? (
                    <>
                      <input type="number" value={editThreshold} onChange={(e) => setEditThreshold(e.target.value)}
                        className="w-16 bg-[#F0F2F8] border border-[#C9A84C] rounded px-2 py-1 text-xs text-[#0D1B3E] outline-none" min={0} />
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full w-fit ${status.color}`}>{status.label}</span>
              </div>
            )
          })
        )}
          ...
          {productSales.length === 0 ? (
            ...
            productSales.map((p, i) => (
              <div key={p.product_id} className="grid grid-cols-5 px-4 py-3 border-b border-[#0D1B3E]/5 hover:bg-[#F0F2F8]/50 items-center">
                ...
                <p className="text-xs font-semibold text-[#2563eb]">₱{Number(p.revenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                <p className="text-xs text-[#e05252]">₱{Number(p.cost || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                <p className="text-xs font-semibold text-[#1a7a4a]">₱{Number(p.profit || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              </div>
            ))
          )}
          {productSales.length > 0 && (
            <div className="grid grid-cols-5 px-4 py-3 bg-[#F0F2F8] font-semibold border-t border-[#0D1B3E]/8">
              ...
              <p className="text-xs text-[#2563eb]">₱{productSales.reduce((s, p) => s + Number(p.revenue || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-[#e05252]">₱{productSales.reduce((s, p) => s + Number(p.cost || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-[#1a7a4a]">₱{productSales.reduce((s, p) => s + Number(p.profit || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
          )}
        </div>
      )}
