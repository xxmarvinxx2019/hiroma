...
const navItems = [
  {
    section: 'Main',
    items: [
      { label: 'Dashboard', href: '/dashboard/regional', icon: '📊' },
    ],
  },
  {
    section: 'Distribution',
    items: [
      { label: 'Inventory',    href: '/dashboard/regional/inventory',    icon: '📦' },
      { label: 'Orders',       href: '/dashboard/regional/orders',       icon: '🛒' },
      { label: 'Payment Methods', href: '/dashboard/regional/payment-methods', icon: '💳' },
    ],
  },
  {
    section: 'Network',
  ...
  },
  {
    section: 'Account',
    items: [
      { label: 'Digital ID', href: '/dashboard/regional/digital-id', icon: 'ID' },
    ],
  },
]
...
function Sidebar({
  ...
  user: { id: string; full_name: string; username: string; profile_photo?: string | null; distributor_profile?: { coverage_area: string } } | null
  pathname: string
  onClose: () => void
  onLogout: () => void
}) {
  const isActive = (href: string) => {
    if (href === '/dashboard/regional') return pathname === href
      ...
      <div className="px-3 py-3 border-t border-white/5 flex-shrink-0 bg-[#010521]">
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <div className="overflow-hidden">
            <p className="text-white text-xs font-medium truncate">{user?.full_name || 'Regional Dist.'}</p>
            <p className="text-white/40 text-xs truncate">
              {user?.distributor_profile?.coverage_area || 'Regional distributor'}
            </p>
          </div>
        </div>
...
export default function CityLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentLabel =
    navItems
      .flatMap((g) => g.items)
      .find((i) =>
        i.href === '/dashboard/regional' ? pathname === i.href : pathname.startsWith(i.href)
      )?.label || 'Dashboard'
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#F0F2F8' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <header className="bg-[#010521] flex items-center justify-between px-4 border-b border-white/5 flex-shrink-0" style={{ height: '56px' }}>
          ...
          <div className="flex items-center gap-3">
            <NotificationBell userId={user?.id} role="regional" />
                ...
                <>
                  ...
                  <div className="absolute right-0 top-10 z-50 bg-white rounded-2xl shadow-xl border border-[#0D1B3E]/8 w-52 overflow-hidden">
                    ...
                    <div className="py-1">
                      ...
                      <Link href="/dashboard/regional/digital-id" onClick={() => setProfileMenuOpen(false)}
                        className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#f8f9fc] transition-colors">
                      ...
                      <Link href="/dashboard/regional/settings" onClick={() => setProfileMenuOpen(false)}
                        className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#f8f9fc] transition-colors">
