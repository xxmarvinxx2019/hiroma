...
export default function RegionalInventoryPage() {
  ...
  const saveThreshold = async (id: string) => {
    setSaving(true)
    await fetch('/api/regional/inventory', {
      method:  'PATCH',
    ...
    <div className="w-full space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Manage your stock levels and track product movement</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Stock Value',      value: fmtS(summary.total_sell_value),              color: '#1a7a4a', icon: '💰', sub: `Cost: ${fmtS(summary.total_cost_value)}` },
        ].map(s => (
            ...
            return (
              <div key={item.id} className={`grid grid-cols-6 px-4 py-3.5 border-b border-[#0D1B3E]/5 hover:bg-[#f8f9fc] transition-colors items-center ${!item.product.is_active ? 'opacity-50' : ''}`}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400">{fmt(item.product.regional_price)} / unit</p>
                </div>
                {/* Type */}
