...
export default function RegionalInventoryPage() {
  ...
  const fetchItems = useCallback(() => {
    ...
    fetch(`/api/regional/inventory?${params}`)
      .then((r) => r.json())
      .then((data) => {
        setItems(data.items || [])
  ...
  const handleSaveThreshold = async (id: string) => {
    setSaving(true)
    const res = await fetch('/api/regional/inventory', {
      method: 'PATCH',
    ...
    <div className="max-w-7xl mx-auto">
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Cost: ₱{Number(item.product.regional_price).toLocaleString()} · Sell: ₱{Number(item.product.provincial_price).toLocaleString()}</p>
                </div>
      ...
      {tab === 'inventory' && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          {[
            { label: 'Stock at Regional Price',    value: summary.total_cost_value,  accent: '#0D1B3E', hint: 'What you paid (regional price)' },
            { label: 'Stock at Provincial Price', value: summary.total_sell_value,  accent: '#2563eb', hint: 'What provincial distributors pay you' },
