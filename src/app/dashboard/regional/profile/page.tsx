...
export default function RegionalProfilePage() {
  ...
  const handleProfileSave = async () => {
    ...
    setProfileError('')
    setProfileSuccess('')
    const res = await fetch('/api/regional/profile', {
      method: 'PATCH',
  ...
  const handlePasswordSave = async () => {
    ...
    const res = await fetch('/api/regional/profile/password', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Section title="Account Info" desc="Your system credentials and coverage area">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <InputField label="Role"          value="Regional Distributor"     disabled />
