  ...
  try {
    ...
    const all = await prisma.inventory.findMany({
      ...
      select: {
        ...
        product: { select: { regional_price: true, provincial_price: true } },
      },
    })
    ...
    const actualCost = deliveredOrders.reduce(
      (s, o) => s + o.items.reduce((ss, i) => ss + Number(i.product.regional_price) * i.quantity, 0), 0
    )
    ...
    for (const o of deliveredOrders) {
      for (const item of o.items) {
        ...
        existing.cost       += Number(p.regional_price) * item.quantity
    ...
    const summary = {
      ...
      total_cost_value:  all.reduce((s, i) => s + Number((i.product as any).regional_price)   * i.quantity, 0),
      total_sell_value:  all.reduce((s, i) => s + Number((i.product as any).provincial_price) * i.quantity, 0),
      potential_profit:  all.reduce((s, i) => s + (Number((i.product as any).provincial_price) - Number((i.product as any).regional_price)) * i.quantity, 0),
    ...
    console.error('[REGIONAL INVENTORY ERROR]', error)
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    console.error('[REGIONAL INVENTORY PATCH ERROR]', error)
    return NextResponse.json({ error: 'Something went wrong.' }, { status: 500 })
  }
}