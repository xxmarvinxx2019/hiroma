...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get regional's own distributor profile
    const myProfile = await prisma.distributorProfile.findUnique({
      where:  { user_id: user.id },
      select: { id: true, coverage_area: true, region_code: true },
    })
    ...
    if (myProfile.region_code) {
      orConditions.push({ region_code: myProfile.region_code })
    }
    ...
    console.error('[REGIONAL DISTRIBUTORS ERROR]', error)
