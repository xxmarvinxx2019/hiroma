...
export async function POST(req: NextRequest) {
  try {
    ...
    if (pin.city_dist_id !== user.id)
      return NextResponse.json(
        { error: "This PIN does not belong to admin." },
        { status: 400 },
      );
    if (slotTaken)
      return NextResponse.json(
        { error: "Slot already taken. Please refresh." },
        { status: 400 },
      );
    if (!parentNodeExists)
      return NextResponse.json(
    ...
    if (!isHiromaNode && referrerProfile) {
      ...
      `;
      overflowToHiroma =
        Boolean(capConfig?.enabled) && count >= Number(capConfig?.cap || 10);
    }
    ...
    const registration = await prisma.$transaction(async (tx) => {
      ...
      await tx.registrationFinancial.create({
        data: {
          ...
          binary_commission_allocation:
            Number(packageSnapshot.pairing_bonus_value) * 0.5,
          binary_points_per_pair: Math.round(
            Number(packageSnapshot.pairing_bonus_value),
          ),
      ...
      const binaryCommission = await settleBinaryCommission(tx, {
        ...
        sourcePoints: Math.round(Number(packageSnapshot.pairing_bonus_value)),
    ...
    try {
      const ancestors = await prisma.$queryRaw<
        { id: string; parent_id: string | null; position: string | null }[]
      >`
      ...
      await Promise.all(
        ancestors.map((node, i) => {
          const side =
            i === 0 ? actual_position : (ancestors[i - 1].position as string);
      ...
      console.error("[ADMIN REGISTER] Ancestor count error:", e);
    }

    // Direct referral bonus — fires for everyone, overflow goes to Hiroma if cap exceeded
    if (false && !isHiromaNode) {
      try {
        ...
        if (overflowToHiroma) {
          ...
          // Normal — update daily count, credit referrer
          ...
          const bonusOps: Promise<unknown>[] = [];
          if (earned > 0) {
            bonusOps.push(
        ...
        console.error("[ADMIN REGISTER] Direct referral error:", e);
      }
    }
    ...
    try {
      ...
      if (false && newUserPts > 0) {
        ...
        const ancestorPackageIds = [
          ...new Set(
            ancestorProfiles.map((p) => p.package?.id).filter(Boolean),
          ),
        ] as string[];
        const binaryCaps = ancestorPackageIds.length
          ? await prisma.$queryRaw<
              { id: string; enabled: boolean; cap: number }[]
            >`
              SELECT id::text,
        ...
        for (let i = 0; i < ancestors.length; i++) {
          ...
          const ancestorPkgPts = Number(
            profile.package?.pairing_bonus_value || 0,
          );
          if (ancestorPkgPts <= 0) {
            currentLeg = (ancestor.position as "left" | "right") || currentLeg;
            continue;
          ...
          const isToday = profile.daily_pairing_date
            ? new Date(profile.daily_pairing_date) >= today
            : false;
          ...
          if (possiblePairs > 0) {
            ...
            const paidEarnings = paidPairs * pointsPerPair * 0.5;
            const overflowEarnings = overflowPairs * pointsPerPair * 0.5;
      ...
      console.error("[ADMIN REGISTER] Binary pairing error:", e);
    }
    ...
    return NextResponse.json({
      ...
      package: packageWithProducts
        ? {
            ...
            products_total: packageWithProducts.products.reduce(
              (s, p) => s + Number(p.product.price || 0) * p.quantity,
              0,
            ),
            reseller_value: registrationEconomics.resellerValue,
