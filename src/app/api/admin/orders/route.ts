...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (status) {
        const claimed = await tx.order.updateMany({
          ...
          data: { status, ...(payment_status && { payment_status }), ...(status === 'cancelled' && buildOrderCancellationEvidence(user, cancellation_reason, 'admin')) },
        })
    ...
    const buyerRole = (await prisma.user.findUnique({
      where: { id: order.buyer_id },
      select: { role: true },
    }))?.role

    // The database qualifies Product Binary only after both facts are true.
    // This also settles an order that was delivered first and paid later.
    if (updated.status === 'delivered' && buyerRole === 'reseller' && updated.payment_status !== 'paid') {
      rewardsPending = true
      rewardsWarning = PRODUCT_BINARY_WAITING_PAYMENT_WARNING
    } else if (updated.status === 'delivered' && updated.payment_status === 'paid') {
      try {
        ...
        console.error('[ADMIN ORDERS] Product binary pairing error:', e)
      }
    }
