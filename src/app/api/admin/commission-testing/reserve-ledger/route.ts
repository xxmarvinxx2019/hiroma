  ...
  const [direct,binary,product,movements]=await Promise.all([
   prisma.$queryRaw<Record<string, unknown>[]>`
    SELECT
     ...
     (COALESCE((SELECT SUM(original_amount) FROM direct_referral_reserve_lots WHERE allocated_at<=${to}),0)-COALESCE((SELECT SUM(c.amount) FROM direct_referral_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)<=${to}),0))::float liability,
     COALESCE((SELECT SUM(c.amount) FROM direct_referral_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)>=${from} AND COALESCE(p.processed_at,p.requested_at)<=${to}),0)::float paid,
   ...
   prisma.$queryRaw<Record<string, unknown>[]>`
    SELECT COALESCE((SELECT SUM(original_amount) FROM binary_reserve_lots WHERE allocated_at<=${to}),0)::float allocated,
     ...
     (COALESCE((SELECT SUM(original_amount) FROM binary_payable_lots WHERE allocated_at<=${to}),0)-COALESCE((SELECT SUM(c.amount) FROM binary_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)<=${to}),0))::float liability,
     COALESCE((SELECT SUM(c.amount) FROM binary_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)>=${from} AND COALESCE(p.processed_at,p.requested_at)<=${to}),0)::float paid,
   ...
   prisma.$queryRaw<Record<string, unknown>[]>`
    SELECT COALESCE((SELECT SUM(original_amount) FROM product_binary_funding_lots WHERE allocated_at<=${to}),0)::float allocated,
     ...
     (COALESCE((SELECT SUM(original_amount) FROM product_binary_payable_lots WHERE allocated_at<=${to}),0)-COALESCE((SELECT SUM(c.amount) FROM product_binary_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)<=${to}),0))::float liability,
     COALESCE((SELECT SUM(c.amount) FROM product_binary_payout_consumptions c JOIN payouts p ON p.id=c.payout_id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)>=${from} AND COALESCE(p.processed_at,p.requested_at)<=${to}),0)::float paid,
   ...
   prisma.$queryRaw<Record<string, unknown>[]>`
    WITH entries AS (
     ...
     UNION ALL SELECT COALESCE(p.processed_at,p.requested_at),CASE WHEN d.payout_id IS NOT NULL THEN 'direct_referral' WHEN b.payout_id IS NOT NULL THEN 'binary' ELSE 'product_binary' END,'released',p.id,p.user_id,COALESCE(d.amount,b.amount,pr.amount)::float,'Released payout allocation' FROM payouts p LEFT JOIN direct_referral_payout_consumptions d ON d.payout_id=p.id LEFT JOIN binary_payout_consumptions b ON b.payout_id=p.id LEFT JOIN product_binary_payout_consumptions pr ON pr.payout_id=p.id WHERE p.status='released' AND COALESCE(p.processed_at,p.requested_at)>=${from} AND COALESCE(p.processed_at,p.requested_at)<=${to}
