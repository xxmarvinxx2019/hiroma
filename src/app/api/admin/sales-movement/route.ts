...
type Summary = {
  ...
  collected: number;
  receivable: number;
  admin_gross_sales: number;
  admin_cost: number;
  admin_profit: number;
  exact_cost_rows: number;
  fallback_cost_rows: number;
};
type Breakdown = {
  key: string;
  label: string;
...
export async function GET(req: NextRequest) {
  ...
  const search = (req.nextUrl.searchParams.get("search") || "")
    ...
    .toLowerCase()
    .slice(0, 100);
  const distributorId = (req.nextUrl.searchParams.get("distributor_id") || "")
    .trim()
    .slice(0, 80);
  ...
  const regionCode = (req.nextUrl.searchParams.get("region_code") || "")
    .trim()
    .slice(0, 30);
  const provinceCode = (req.nextUrl.searchParams.get("province_code") || "")
    .trim()
    .slice(0, 30);
  const cityCode = (req.nextUrl.searchParams.get("city_code") || "")
    .trim()
    .slice(0, 30);
  ...
  const requestedSize = Number.parseInt(
    req.nextUrl.searchParams.get("page_size") || "50",
    10,
  );
  const page =
    Number.isSafeInteger(requestedPage) && requestedPage > 0
      ? requestedPage
      : 1;
  const pageSize = Number.isSafeInteger(requestedSize)
  ...
  try {
    const [
      ...
      `,
      prisma.$queryRaw<AreaProductBreakdown[]>`
        WITH x AS (
          SELECT p.id product_id,p.name product_name,p.type::text product_type,o.id order_id,o.seller_id,o.buyer_id,oi.quantity,
            oi.subtotal::float gross_sales,(COALESCE(oi.unit_acquisition_cost,im.unit_cost,p.cost_price)*oi.quantity)::float cost,o.payment_status
          FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN products p ON p.id=oi.product_id
          JOIN distributor_profiles dp ON dp.user_id=o.seller_id
          LEFT JOIN LATERAL (SELECT m.unit_cost FROM inventory_movements m WHERE m.order_id::text=o.id::text AND m.product_id::text=oi.product_id::text ORDER BY m.created_at DESC LIMIT 1) im ON true
          WHERE o.status='delivered' AND COALESCE(o.delivered_at,o.created_at)>=${from} AND COALESCE(o.delivered_at,o.created_at)<=${to}
            AND (${regionCode}='' OR dp.region_code=${regionCode}) AND (${provinceCode}='' OR dp.province_code=${provinceCode}) AND (${cityCode}='' OR dp.city_muni_code=${cityCode})
            AND (${productId}='' OR oi.product_id::text=${productId})
        ) SELECT product_id,product_name,product_type,COUNT(DISTINCT seller_id)::int sellers,COUNT(DISTINCT buyer_id)::int buyers,
          COUNT(DISTINCT order_id)::int orders,SUM(quantity)::int units,SUM(gross_sales)::float gross_sales,SUM(cost)::float cost,
          SUM(gross_sales-cost)::float gross_profit,COALESCE(SUM(gross_sales) FILTER(WHERE payment_status='paid'),0)::float collected,
          COALESCE(SUM(gross_sales) FILTER(WHERE COALESCE(payment_status,'unpaid')<>'paid'),0)::float receivable
        FROM x GROUP BY product_id,product_name,product_type ORDER BY units DESC,gross_sales DESC,product_name
      `,
      prisma.$queryRaw<AreaAnalysis[]>`
        WITH x AS (
          SELECT o.id,o.seller_id,o.buyer_id,oi.quantity,oi.subtotal::float gross_sales,o.payment_status,
            o.market_region_code region_code,o.market_region_name region_name,o.market_province_code province_code,
            o.market_province_name province_name,o.market_city_muni_code city_muni_code,o.market_city_muni_name city_muni_name
          FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN users buyer ON buyer.id=o.buyer_id
          WHERE o.status='delivered' AND buyer.role='reseller'
            AND COALESCE(o.delivered_at,o.created_at)>=${from} AND COALESCE(o.delivered_at,o.created_at)<=${to}
            AND o.market_region_code IS NOT NULL
            AND (${regionCode}='' OR o.market_region_code=${regionCode})
            AND (${provinceCode}='' OR o.market_province_code=${provinceCode})
            AND (${cityCode}='' OR o.market_city_muni_code=${cityCode})
            AND (${productId}='' OR oi.product_id::text=${productId})
        ) SELECT CONCAT_WS(':',COALESCE(region_code,''),COALESCE(province_code,''),COALESCE(city_muni_code,'')) key,
          region_code,region_name,province_code,province_name,city_muni_code,city_muni_name,
          COUNT(DISTINCT seller_id)::int sellers,COUNT(DISTINCT buyer_id)::int buyers,COUNT(DISTINCT id)::int orders,SUM(quantity)::int units,
          SUM(gross_sales)::float gross_sales,0::float cost,0::float gross_profit,
          COALESCE(SUM(gross_sales) FILTER(WHERE payment_status='paid'),0)::float collected,
          COALESCE(SUM(gross_sales) FILTER(WHERE COALESCE(payment_status,'unpaid')<>'paid'),0)::float receivable
        FROM x GROUP BY region_code,region_name,province_code,province_name,city_muni_code,city_muni_name ORDER BY units DESC,gross_sales DESC
      `,
      prisma.$queryRaw<AreaProductBreakdown[]>`
        WITH x AS (
          SELECT p.id product_id,p.name product_name,p.type::text product_type,o.id order_id,o.seller_id,o.buyer_id,
            oi.quantity,oi.subtotal::float gross_sales,o.payment_status
          FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN products p ON p.id=oi.product_id JOIN users buyer ON buyer.id=o.buyer_id
          WHERE o.status='delivered' AND buyer.role='reseller'
            AND COALESCE(o.delivered_at,o.created_at)>=${from} AND COALESCE(o.delivered_at,o.created_at)<=${to}
            AND o.market_region_code IS NOT NULL
            AND (${regionCode}='' OR o.market_region_code=${regionCode})
          ...
          COUNT(DISTINCT order_id)::int orders,SUM(quantity)::int units,SUM(gross_sales)::float gross_sales,0::float cost,0::float gross_profit,
          COALESCE(SUM(gross_sales) FILTER(WHERE payment_status='paid'),0)::float collected,
          COALESCE(SUM(gross_sales) FILTER(WHERE COALESCE(payment_status,'unpaid')<>'paid'),0)::float receivable
        FROM x GROUP BY product_id,product_name,product_type ORDER BY units DESC,gross_sales DESC,product_name
      `,
      prisma.$queryRaw<MarketSupplierBreakdown[]>`
        SELECT seller.id seller_id,seller.full_name seller_name,seller.username seller_username,
          CASE WHEN dp.dist_level='branch' THEN 'branch' ELSE seller.role::text END seller_level,
          o.fulfillment_region_name region_name,o.fulfillment_province_name province_name,o.fulfillment_city_muni_name city_muni_name,
          COUNT(DISTINCT o.id)::int orders,COUNT(DISTINCT o.buyer_id)::int resellers,SUM(oi.quantity)::int units,SUM(oi.subtotal)::float gross_sales,
          COALESCE(SUM(oi.quantity) FILTER(WHERE o.market_city_muni_code IS NOT NULL AND o.market_city_muni_code=o.fulfillment_city_muni_code),0)::int local_units,
          COALESCE(SUM(oi.quantity) FILTER(WHERE o.market_city_muni_code IS DISTINCT FROM o.fulfillment_city_muni_code),0)::int outside_units
        FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN users buyer ON buyer.id=o.buyer_id JOIN users seller ON seller.id=o.seller_id
        LEFT JOIN distributor_profiles dp ON dp.user_id=seller.id
        WHERE o.status='delivered' AND buyer.role='reseller'
          AND COALESCE(o.delivered_at,o.created_at)>=${from} AND COALESCE(o.delivered_at,o.created_at)<=${to}
          AND o.market_region_code IS NOT NULL
          AND (${regionCode}='' OR o.market_region_code=${regionCode})
          ...
          AND (${cityCode}='' OR o.market_city_muni_code=${cityCode})
          AND (${productId}='' OR oi.product_id::text=${productId})
        GROUP BY seller.id,seller.full_name,seller.username,seller.role,dp.dist_level,o.fulfillment_region_name,o.fulfillment_province_name,o.fulfillment_city_muni_name
        ORDER BY units DESC,gross_sales DESC,seller.full_name
      `,
      prisma.$queryRaw<MarketAreaDirectory[]>`
        SELECT DISTINCT region_code,region_name,province_code,province_name,city_muni_code,city_muni_name
        FROM users WHERE role='reseller' AND region_code IS NOT NULL
        ORDER BY region_name,province_name,city_muni_name
      `,
    ]);
    const summary = summaryRows[0] || ({} as Summary);
    const activation = activationRows[0] || ({} as Activation);
    const distributorAnalysis =
      distributorAnalysisRows[0] || ({} as DistributorAnalysis);
    const selectedDistributor =
      distributorDirectory.find((item) => item.id === distributorId) || null;
    const areaDirectory = distributorDirectory.filter((item) => {
      const profile = item.distributor_profile;
      return (
        profile &&
        (!regionCode || profile.region_code === regionCode) &&
        (!provinceCode || profile.province_code === provinceCode) &&
        (!cityCode || profile.city_muni_code === cityCode)
      );
    });
    const areaCoverage = areaDirectory.reduce(
      (counts, item) => {
    ...
    const areaSummary = areaRows.reduce(
      (total, row) => ({
        sellers: total.sellers + num(row.sellers),
        buyers: total.buyers + num(row.buyers),
        orders: total.orders + num(row.orders),
        units: total.units + num(row.units),
        gross_sales: total.gross_sales + num(row.gross_sales),
        cost: total.cost + num(row.cost),
        gross_profit: total.gross_profit + num(row.gross_profit),
        collected: total.collected + num(row.collected),
        receivable: total.receivable + num(row.receivable),
      }),
      {
        sellers: 0,
        buyers: 0,
    ...
    const marketSummary = marketAreaRows.reduce(
      (total, row) => ({
        sellers: total.sellers + num(row.sellers),
        buyers: total.buyers + num(row.buyers),
        orders: total.orders + num(row.orders),
    ...
    const localDemandUnits = marketSupplierRows.reduce(
      (sum, row) => sum + num(row.local_units),
      0,
    );
    const outsideDemandUnits = marketSupplierRows.reduce(
      (sum, row) => sum + num(row.outside_units),
      0,
    );
    marketSummary.sellers = new Set(
      marketSupplierRows.map((row) => row.seller_id),
    ).size;
    ...
    return NextResponse.json({
      ...
      summary: Object.fromEntries(
        Object.entries(summary).map(([key, value]) => [key, num(value)]),
      ),
      routes: routeRows,
      liquidation: sellerRows,
      products: productRows,
      movements: movementRows,
      activation: Object.fromEntries(
        Object.entries(activation).map(([key, value]) => [key, num(value)]),
      ),
      activation_breakdown: activationBreakdown,
      directory: {
        distributors: distributorDirectory,
        products: productDirectory,
      },
      selected_distributor: selectedDistributor,
      distributor_analysis: Object.fromEntries(
        Object.entries(distributorAnalysis).map(([key, value]) => [
          key,
          num(value),
        ]),
      ),
      distributor_trend: trendRows,
      area_summary: areaSummary,
      area_analysis: areaRows,
      area_trend: areaTrendRows,
      area_products: areaProductRows,
      area_coverage: areaCoverage,
      market_summary: {
        ...marketSummary,
        local_units: localDemandUnits,
        outside_units: outsideDemandUnits,
      },
      market_analysis: marketAreaRows,
      market_products: marketProductRows,
      market_suppliers: marketSupplierRows,
      market_areas: marketAreaDirectory,
      pagination: {
        page,
        page_size: pageSize,
      ...
      notes: {
        chain_turnover:
          "Network turnover counts each delivered seller-to-buyer transaction. Do not treat it as unique end-customer revenue.",
        company_sales:
          "Admin product sales are the delivered transactions where Admin is the seller. Activation PIN allocation is reported separately to prevent double counting.",
        ...
        read_only:
          "This module is read-only and does not change orders, inventory, PINs, wallets, or commissions.",
      },
    });
  } catch (error) {
    console.error("[SALES MOVEMENT]", error);
    return NextResponse.json(
      { error: "Unable to load sales and product movement." },
      { status: 500 },
    );
  }
}
