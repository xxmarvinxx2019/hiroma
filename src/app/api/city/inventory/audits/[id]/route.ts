...
import { returnPosShiftForRecount, PosShiftRecountConflictError } from '@/app/lib/posShiftRecount'
  ...
  try {
    ...
    if (action === 'reject') {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (session.pos_shift_id) {
          try {
            await returnPosShiftForRecount(tx, { shiftId: session.pos_shift_id, ownerId: user.id, notes })
          } catch (error) {
            if (error instanceof PosShiftRecountConflictError) throw new InventoryAuditConflictError(error.message)
            throw error
          }
        }
      })
