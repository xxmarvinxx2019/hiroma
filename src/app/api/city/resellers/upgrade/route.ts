...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const oldQty = new Map(
      resellerProfile.package!.products.map((item) => [
        item.product_id,
        item.quantity,
      ]),
    );
    const extraProducts = pin
      .package!.products.map((item) => ({
        product_id: item.product_id,
        quantity: item.quantity - (oldQty.get(item.product_id) || 0),
      }))
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.binaryReserveLot.create({
        data: {
          registration_financial_id: null,
          upgrade_financial_id: financial.id,
          original_amount: new Prisma.Decimal(
            pin.upgrade_binary_allocation_snapshot!,
          ),
          remaining_amount: new Prisma.Decimal(
            pin.upgrade_binary_allocation_snapshot!,
          ),
          snapshot_source: "upgrade_pin_snapshot",
          allocated_at: now,
        },
      });
...
// Retained as a compatibility fallback for the legacy pairing path.
void fireUpgradePairingBonus;
