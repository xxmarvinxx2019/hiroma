  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const shift = await tx.posShift.findFirst({
        where: { id: shiftId, owner_id: user.id, opened_by_id: actorId, status: { in: isBranch ? ['open', 'needs_review'] : ['open'] } },
        select: { id: true, terminal_id: true, opening_cash: true, closing_count_attempts: true },
      })
      ...
      if (isBranch) {
        if (!Array.isArray(body.inventory_counts)) return { inventoryRequired: true as const }
        const inventory = await tx.inventory.findMany({
          where: { owner_id: user.id },
          orderBy: { product: { name: 'asc' } },
          include: { product: { select: { id: true, name: true, branch_price: true, cost_price: true } } },
        })
        if (inventory.length === 0) return { inventoryEmpty: true as const }
        const submitted = new Map<string, { counted: number; damaged: number; expired: number }>()
        for (const raw of body.inventory_counts as Array<Record<string, unknown>>) {
          ...
          if (!productId || submitted.has(productId) || counted === null || damaged + expired > counted) return { invalidInventory: true as const }
          submitted.set(productId, { counted, damaged, expired })
        }
        if (submitted.size !== inventory.length || inventory.some((item) => !submitted.has(item.product_id))) return { incompleteInventory: true as const }
        ...
        const audit = await tx.inventoryAuditSession.upsert({
          ...
          update: {
            ...
            items: {
              ...
              create: rows.map((row) => ({
                ...
                unit_cost_snapshot: new Prisma.Decimal(Number(row.item.product.branch_price) || Number(row.item.product.cost_price)),
                variance_value: new Prisma.Decimal(row.variance * (Number(row.item.product.branch_price) || Number(row.item.product.cost_price))),
          ...
          create: {
            ...
            scope: 'shift_closing',
            started_by: actorId,
            started_by_name_snapshot: user.actor_name || user.full_name || user.username,
            notes: explanation || 'Branch end-of-shift blind count',
            submitted_at: new Date(),
            items: {
              create: rows.map((row) => ({
        ...
        await tx.inventoryAuditEvent.create({
          data: {
            ...
            reason: explanation || 'Branch end-of-shift cash and inventory count submitted',
            metadata: { shift_id: shift.id, terminal_id: shift.terminal_id, mismatch, recount_confirmed: recountConfirmed },
          },
        })
        ...
        return { branchSubmitted: true as const, shift: submittedShift, audit }
      }
    ...
    if ('inventoryRequired' in result) return NextResponse.json({ error: 'A complete physical inventory count is required before this Branch shift can be submitted.', code: 'SHIFT_INVENTORY_REQUIRED' }, { status: 400 })
