...
export async function GET() {
  ...
  try {
    ...
    const eligible = canApprove(user) ? [] : await prisma.posTransaction.findMany({
      where: { owner_id: user.id, cashier_id: actorId, status: 'finalized', adjustment_request: null },
      orderBy: { finalized_at: 'desc' },
      take: 50,
      select: {
        id: true, receipt_number: true, transaction_type: true, customer_name_snapshot: true,
        payment_method_snapshot: true, total_snapshot: true, finalized_at: true,
        items: { select: { product_name_snapshot: true, quantity: true } },
      },
    })
    return NextResponse.json({
      ...
      eligible_receipts: eligible.map((row) => ({ ...row, total: Number(row.total_snapshot) })),
  ...
  try {
    ...
    const created = await prisma.posAdjustmentRequest.create({
      data: {
        ...
        reason,
        amount_snapshot: transaction.total_snapshot,
      },
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') return NextResponse.json({ error: 'This receipt already has a void or refund request. Review its existing status instead of creating a duplicate.' }, { status: 409 })
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const request = await tx.posAdjustmentRequest.findFirst({
        ...
        include: { transaction: { include: { items: true, order: { select: { id: true } } } } },
      })
      ...
      if (action === 'approve') {
        const nextStatus = request.request_type === 'void' ? 'voided' : 'refunded'
        const transactionClaim = await tx.posTransaction.updateMany({
          where: { id: request.transaction.id, status: 'finalized' },
          data: { status: nextStatus, reviewed_by_id: actorId, reviewed_at: new Date(), review_notes: notes },
        })
        if (transactionClaim.count !== 1) throw new Error('POS_TRANSACTION_NOT_ADJUSTABLE')
        for (const item of request.transaction.items) {
          ...
          const after = await tx.inventory.update({ where: { owner_id_product_id: { owner_id: user.id, product_id: item.product_id } }, data: { quantity: { increment: item.quantity } }, select: { quantity: true } })
          await tx.inventoryAuditEvent.create({
            data: {
              ...
              event_type: request.request_type === 'void' ? 'pos_void_return' : 'pos_refund_return',
              quantity_delta: item.quantity, quantity_before: before.quantity, quantity_after: after.quantity,
              unit_cost_snapshot: item.unit_cost_snapshot, total_value: Number(item.unit_cost_snapshot) * item.quantity,
              ...
              metadata: { pos_transaction_id: request.transaction.id, adjustment_request_id: request.id, original_receipt: request.transaction.receipt_number },
            },
          })
        }
