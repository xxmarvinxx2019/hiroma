...
import { useLiveConversation, useConversationScroll } from "@/app/components/useLiveConversation"
...
export default function OrderConversation({ orderId }: { orderId: string }) {
  ...
  const conversationScroll = useConversationScroll(messages.at(-1)?.id)
  const reconnecting = useLiveConversation<{ messages: Message[]; closed: boolean; accountId: string }>(`/api/orders/${orderId}/messages`, data => { setMessages(data.messages || []); setClosed(Boolean(data.closed)); setAccountId(data.accountId || ""); setLoading(false) })
